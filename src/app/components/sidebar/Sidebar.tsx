...
interface SidebarProps {
  ...
  activeTab: "chat" | "commands" | "avatar";
  setActiveTab: (tab: "chat" | "commands" | "avatar") => void;
  barPosition: number;
...
export default function Sidebar({
  ...
  return (
    <aside
      className="w-64 bg-[#18181b] border-r border-[#3f3f46] p-6 flex flex-col justify-between"
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-3 px-3 py-2 bg-blue-600/10 border border-blue-600 rounded-lg w-full">
          <div className="w-10 h-10 bg-blue-600 rounded-lg flex items-center justify-center flex-shrink-0">
            <img src="/logo.png" alt="RawenChat Logo" className="w-6 h-6" />
          </div>
        ...
        <nav className="space-y-2 text-sm">
          {!channel && (
            <div
              className={`px-3 py-2.5 rounded-lg transition-all cursor-pointer font-medium bg-blue-600 text-white shadow-lg flex items-center gap-3`}
            >
          ...
          {channel && (
            <div className="relative" ref={navRef}>
              <div
                className={`absolute -left-4 top-0 w-1 h-10 bg-gradient-to-b from-blue-600 to-cyan-600 rounded-full transition-all duration-300`}
              ...
              <div className="space-y-3">
                ...
                >
                  <PersonAccounts20Regular className="w-5 h-5" />
                  <span>Avatar</span>
                </div>
              </div>
            </div>
          ...
          <>
            <button
              onClick={() => {
                ...
                const obsUrl = `${origin}/obs?channel=${encodeURIComponent(channel)}&platform=${platform}`;
                navigator.clipboard.writeText(obsUrl);
                setToastMessage("URL de Chat Overlay copiado");
              }}
              className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-purple-950/30 hover:bg-purple-950/50 transition-all text-purple-400 font-medium w-full border border-purple-900/50 cursor-pointer"
              title="Copiar URL del Chat Overlay para OBS"
            >
              <Chat20Regular className="w-5 h-5" />
              <span className="text-sm">Copiar Chat Overlay</span>
            </button>
            <button
              onClick={() => {
                const origin = typeof window !== "undefined" ? window.location.origin : "http://localhost:3000";
                const avatarUrl = `${origin}/avatar`;
                navigator.clipboard.writeText(avatarUrl);
                setToastMessage("URL de Avatar Overlay copiado");
              }}
              className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-[#0e3b43]/30 hover:bg-[#0e3b43]/50 transition-all text-cyan-400 font-medium w-full border border-cyan-900/50 cursor-pointer"
              title="Copiar URL del Avatar Overlay para OBS"
            >
              <PersonAccounts20Regular className="w-5 h-5" />
              <span className="text-sm">Copiar Avatar Overlay</span>
            </button>
          </>
        )}
        {channel && (
